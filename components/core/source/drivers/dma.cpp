...
#if CONFIG_PERIPH_DMA_LOG
#include "common/log_monitor.h"
#endif /* CONFIG_PERIPH_DMA_LOG */
...
err_t dma::initialize(dma_config_t *conf){
	err_t ret;
...
DMA_TypeDef *dma::get_instance(void){
	return _instance;
}
...
uint16_t dma::get_transfer_counter(void){
#if defined(STM32F1)
...
SemaphoreHandle_t dma::get_mutex(void){
	return _mutex;
}


void dma::register_event_handler(std::function<void(dma_event_t, void *)> event_handler_function, void *event_parameter){
	_event_handler = event_handler_function;
	_event_parameter = event_parameter;
}

void dma::unregister_event_handler(void){
	_event_handler = NULL;
	_event_parameter = NULL;
}

void dma::event_handle(dma_event_t event){
	if(_event_handler) _event_handler(event, _event_parameter);
}



void dma::config(uint32_t psource, uint32_t pdest, uint32_t size){
#if defined(STM32F1)
...
err_t dma::config_start_session(struct dma_session_config_t conf){
	err_t ret;
...
err_t dma::wait_for(dma_event_t break_event, uint32_t timeout){
	err_t ret;
...
#if defined(STM32F1)
	if(break_event == DMA_EVENT_TRANFER_COMPLETE)
		PollValue = (uint32_t)(0x02U << (_conf->channel));
	else if(break_event == DMA_EVENT_HALF_TRANFER)
	...
	if(break_event == DMA_EVENT_HALF_TRANFER){
		clear_IFCR((uint32_t)(0x04U << (_conf->channel)));
	}
	else if(break_event == DMA_EVENT_TRANFER_COMPLETE){
...
void dma::clear_IFCR(__IO uint32_t Value){
#if defined(STM32F1)
...
__IO uint32_t dma::get_ISR(void){
	__IO uint32_t isr = 0;
